...
pub mod tests {
    ...
    use crate::{aqua::witness::cli_winess_chain, models::CliArgs};
    ...
    fn test_witness_chain() {
        ...
        cli_winess_chain(cli_args.clone(), aqua_verifier, cli_args.file.unwrap());

        // Optional: Add assertions to verify expected behavior
        // 1. Check if output file was created
        assert!(
            cli_args.output.as_ref().unwrap().exists(),
