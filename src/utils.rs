...
use crate::models::PageDataContainer;
...
pub fn read_aqua_data(path: &PathBuf) -> Result<PageDataContainer, String> {
    let data = fs::read_to_string(path);
    match data {
        Ok(data) =>{
            let res= serde_json::from_str::<PageDataContainer>(&data);
            match res {
                Ok(res_data)=>{
                    Ok(res_data)
                }
